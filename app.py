...
def load_data():
    ...
    Always overwrites session state with saved data — fixes stale/empty data issue.
    """
    # Guard: only load once per session to avoid wiping user changes mid-session
        ...
        if os.path.exists("erp_data.json"):
            try:
                with open("erp_data.json", "r", encoding="utf-8") as f:
                    saved = json.load(f)
                # Set defaults first, then overwrite with saved values
            ...
            for row in records:
                if len(row) >= 2 and row[0]:
                    try:
                        ...
                        pass
            # Always overwrite session state with Google Sheets data
            for key, default_val in DEFAULT_DATA.items():
                st.session_state[key] = saved.get(key, default_val)
            # Load pkg_lines
            for k, v in saved.items():
                if k.startswith("pkg_lines_"):
                    st.session_state[k] = v
        else:
            # Sheet is empty — set defaults
            for key, val in DEFAULT_DATA.items():
                st.session_state[key] = val
    except Exception as _e:
        # On error, set defaults but don't crash
        for key, val in DEFAULT_DATA.items():
            if key not in st.session_state:
                st.session_state[key] = val

    # Ensure any new DEFAULT_DATA keys added in code updates are present
    for key, val in DEFAULT_DATA.items():
        if key not in st.session_state:
            st.session_state[key] = val
